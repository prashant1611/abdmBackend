...
export async function handleDiscover(body) {
  ...
  const [rows] = await pool.query(
    `SELECT reference_number, hi_type, patient_id, abha_address 
     FROM documents 
     WHERE patient_id = ? OR abha_address = ? and carecontextSend = 0`,
    [mr, abhaAddress]
  );
...
export async function handleConfirm(body) {
  try {
    ...
    if ([200, 201, 202].includes(response.status)) {
      ...
      await pool.query(
        `UPDATE documents SET carecontextSend = 1
...
export async function handleNotify(body) {
  ...
  // const response = await axios.post(
  //   'http://localhost:8990/abdmwebhooks/',
  //   body,
  // );
}
        ...
        entry: [
              ...
              section: [
                {
                  ...
                  entry: [
                    {
                      reference: "urn:uuid:medicationrequest-1",
                    },
                  ],
                },
              ],
          ...
          },
          {
            ...
            resource: {
              ...
              period: {
                start: "2025-05-24T09:00:00Z",
                end: "2025-05-24T09:30:00Z",
              },
            },
          },
          {
            ...
            resource: {
              ...
              authoredOn: "2025-05-24T09:00:00Z",
              requester: {
                reference: "urn:uuid:practitioner-1",
              },
              dosageInstruction: [
